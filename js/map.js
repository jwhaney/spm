...
require(["esri/map",
  ...
  "dojo/domReady!"
], function(Map, Renderer, domConstruct, LayerInfo, Scalebar, on, domClass, query, Moveable,
  ...
  TxDOTVectorTileLayer = new VectorTileLayer("https://tiles.arcgis.com/tiles/KTcxiTD9dsQw4r7Z/arcgis/rest/services/TxDOT_Vector_Tile_Basemap_test/VectorTileServer"); //TxDOT Vector Tile Basemap
  ...
  sources.push({
    featureLayer: new FeatureLayer("http://services.arcgis.com/KTcxiTD9dsQw4r7Z/arcgis/rest/services/Texas_Counties_Detailed/FeatureServer/0"),
  ...
  sources.push({
    featureLayer: new FeatureLayer("http://services.arcgis.com/KTcxiTD9dsQw4r7Z/arcgis/rest/services/TxDOT_Highway_Designations/FeatureServer/0"),
    searchFields: ["MO_NBR"],
    displayField: "MO_NBR",
    exactMatch: false,
  ...
  tableText += "<tr><td>Route ID</td><td>${RTE_RB_NM}</td></tr>";
  tableText += "<tr><td>Roadbed Type</td><td>${RDBD_TYPE}</td></tr>";
  tableText += "<tr><td>Name/Number</td><td>${MAP_LBL}</td></tr>";
  tableText += "<tr><td>Begin DFO</td><td>${BEGIN_DFO}</td></tr>";
  tableText += "<tr><td>End DFO</td><td>${END_DFO}</td></tr>";
  tableText += "</table>";
  ...
  roadwaysLyr = new FeatureLayer("http://services.arcgis.com/KTcxiTD9dsQw4r7Z/arcgis/rest/services/TxDOT_Roadways/FeatureServer/0", {
    opacity: 0.0,
    outFields: ["RTE_RB_NM", "RDBD_TYPE", "MAP_LBL", "RTE_PRFX", "BEGIN_DFO", "END_DFO"]
  });
  ...
  map.on("zoom-end", function() {
    if (map.getZoom() < 8) {
      roadwaysLyr.setDefinitionExpression("RTE_PRFX = 'IH'");
      map.addLayer(roadwaysLyr);
    }
    if (map.getZoom() > 7 && map.getZoom() < 9) {
      roadwaysLyr.setDefinitionExpression("RTE_PRFX IN ('IH', 'US')");
      map.addLayer(roadwaysLyr);
    }
    if (map.getZoom() > 8 && map.getZoom() < 10) {
      roadwaysLyr.setDefinitionExpression("RTE_PRFX IN ('IH', 'US', 'SH', 'SL', 'FM', 'RM')");
      map.addLayer(roadwaysLyr);
    }
    if (map.getZoom() > 9 && map.getZoom() < 13) {
      roadwaysLyr.setDefinitionExpression("RTE_PRFX NOT IN ('CS', 'PR')");
      map.addLayer(roadwaysLyr);
    }
    if (map.getZoom() > 12) {
  ...
  function getBasemapLayers() {
    ...
    layer1 = (map.getLayer(map.layerIds[1]));
  }
  ...
  function newCoords() {
    if (document.getElementById("measureTab").style.display == "block" || document.getElementById("sketchTab").style.display == "block") {
      if (xPointArray.length >= 1) {
        ...
        polyline.addPath([xPointArray[xPointArray.length - 1],
  ...
  function changeBaseMap(event) {
    ...
    if (newMap == "esriLightGray") {
      esriLightGray = new VectorTileLayer("https://www.arcgis.com/sharing/rest/content/items/5dd75c1a544b46c3af01ba5736bfdfa0/resources/styles/root.json?f=pjson");
      map.addLayer(esriLightGray);
    }
  ...
  function panTo(theX, theY) {
    var thePt = new Point(theX, theY, new SpatialReference({
      wkid: 4269
    }));
  ...
  function scaleDependentQueries() {
    if (activeOverlay) {
      ...
      if (speedLimitLyr) {
        if (map.getZoom() < 9) {
          speedLimitLyr.setDefinitionExpression("RTE_PRFX_TYPE_CD ='IH'");
        }
        if (map.getZoom() > 8 && map.getZoom() < 11) {
          speedLimitLyr.setDefinitionExpression("RTE_PRFX_TYPE_CD IN ('IH','US')");
        }
        if (map.getZoom() > 10 && map.getZoom() < 13) {
          speedLimitLyr.setDefinitionExpression("RTE_PRFX_TYPE_CD IN ('IH','SH','US','SL')");
        }
        if (map.getZoom() > 12) {
  ...
  function hideAllOverlays() {
    ...
    if (MarkerLyr) {
      MarkerLyr.hide();
      labelLayerMarker.hide();
    }
    if (controlSectionLyr) {
      controlSectionLyr.hide();
      labelLayerCS.hide();
    }
    if (USHouseLyr) {
      USHouseLyr.hide();
    ...
    if (futureTrafficLyr) {
      futureTrafficLyr.hide();
    }
    if (longTermPlanLyr) {
      longTermPlanLyr.hide();
    }
    if (constScheduledLyr) {
      constScheduledLyr.hide();
    }
    if (finalForConstLyr) {
      finalForConstLyr.hide();
    }
    if (underDevLyr) {
      underDevLyr.hide();
    }
    ...
    if (theLayer == "CLEAR") {
      ...
      if (activeOverlaysArray.indexOf("Minute_Orders") > -1) {
        ...
        if (minuteOrderLyr) {
          ...
          minuteOrderLyr = new FeatureLayer(projectsUrl, {
            id: "Minute Orders",
            visible: true,
            outFields: ["RTE_NM", "CTRL_SECT", "MO_NBR", "APRV_DT", "DSPN_DSCR", "AASHTO_APPLN"],
      ...
      if (activeOverlaysArray.indexOf("Speed_Limit") > -1) {
        ...
        if (speedLimitLyr) {
          ...
          tableText = "";
          tableText += "<table border=1 cellspacing=0 cellpadding=3 style='font-size:small'>";
          tableText += "<tr><td>Route ID</td><td>${RTE_DEFN_LN_NM}</td></tr>";
          tableText += "<tr><td>Max Speed</td><td>${MAX_SPD_LMT_MS}</td></tr>";
          tableText += "<tr><td>From DFO</td><td>${ASSET_LN_BEGIN_DFO_MS}</td></tr>";
          tableText += "<tr><td>To DFO</td><td>${ASSET_LN_END_DFO_MS}</td></tr>";
          tableText += "</table>";
          ...
          speedLimitLyr = new FeatureLayer(projectsUrl, {
            id: "Speed_Limit",
            visible: true,
            outFields: ["RTE_DEFN_LN_NM", "MAX_SPD_LMT_MS", "ASSET_LN_BEGIN_DFO_MS", "ASSET_LN_END_DFO_MS"],
            infoTemplate: infoTemplate
          });
      ...
      if (activeOverlaysArray.indexOf("Long_Term_Planning") > -1) {
        ...
        if (longTermPlanLyr) {
          ...
          longTermPlanLyr = new FeatureLayer(projectsUrl, {
            id: "Long_Term_Planning",
            visible: true,
            outFields: ["HIGHWAY_NUMBER", "CONTROL_SECT_JOB", "PROJ_CLASS", "TYPE_OF_WORK", "LAYMAN_DESCRIPTION1", "TPP_WORK_PROGRAM", "TPP_CATEGORY_P2", "TPP_PID", "EST_CONST_COST", "UTP_TOTAL_FUNDING_AVAILABLE", "UTP_STRATEGIC_SCORE", "UTP_TOTAL_SCORE", "PRJ_STATUS"],
            infoTemplate: infoTemplate
          });
        ...
        layerInfo.push({
          layer: longTermPlanLyr,
          title: "TxDOT Projects - Long Term Planning"
        });
      }
      ...
      if (activeOverlaysArray.indexOf("Construction_Scheduled") > -1) {
        ...
        if (constScheduledLyr) {
          ...
          constScheduledLyr = new FeatureLayer(projectsUrl, {
            id: "Construction_Scheduled",
            visible: true,
            outFields: ["HIGHWAY_NUMBER", "CONTROL_SECT_JOB", "PROJ_CLASS", "TYPE_OF_WORK", "LAYMAN_DESCRIPTION1", "TPP_WORK_PROGRAM", "TPP_CATEGORY_P2", "TPP_PID", "EST_CONST_COST", "UTP_TOTAL_FUNDING_AVAILABLE", "UTP_STRATEGIC_SCORE", "UTP_TOTAL_SCORE", "PRJ_STATUS"],
            infoTemplate: infoTemplate
          });
        ...
        layerInfo.push({
          layer: constScheduledLyr,
          title: "TxDOT Projects - Construction Scheduled"
        });
      }
      ...
      if (activeOverlaysArray.indexOf("Finalizing_for_Construction") > -1) {
        ...
        if (finalForConstLyr) {
          ...
          finalForConstLyr = new FeatureLayer(projectsUrl, {
            id: "Finalizing_for_Construction",
            visible: true,
            outFields: ["HIGHWAY_NUMBER", "CONTROL_SECT_JOB", "PROJ_CLASS", "TYPE_OF_WORK", "LAYMAN_DESCRIPTION1", "TPP_WORK_PROGRAM", "TPP_CATEGORY_P2", "TPP_PID", "EST_CONST_COST", "UTP_TOTAL_FUNDING_AVAILABLE", "UTP_STRATEGIC_SCORE", "UTP_TOTAL_SCORE", "PRJ_STATUS"],
            infoTemplate: infoTemplate
          });
        ...
        layerInfo.push({
          layer: finalForConstLyr,
          title: "TxDOT Projects - Finalizing for Construction"
        });
      }
      ...
      if (activeOverlaysArray.indexOf("Under_Development") > -1) {
        ...
        if (underDevLyr) {
          ...
          underDevLyr = new FeatureLayer(projectsUrl, {
        ...
        layerInfo.push({
          layer: underDevLyr,
          title: "TxDOT Projects - Under Development"
        });
      }
      ...
      if (activeOverlaysArray.indexOf("Future_Traffic") > -1) {
        ...
        if (futureTrafficLyr) {
          ...
          theRenderer.addBreak({
            minValue: -Infinity,
            maxValue: 50000,
            symbol: (new esri.symbol.CartographicLineSymbol(esri.symbol.CartographicLineSymbol.STYLE_SOLID, new esri.Color("#FF0000"), 1.5, esri.symbol.CartographicLineSymbol.CAP_ROUND, esri.symbol.CartographicLineSymbol.JOIN_MITER, 5)),
            label: "<= 40,000",
            field: "ADT_CUR"
          });
          theRenderer.addBreak({
            minValue: 50000,
            maxValue: 100000,
            symbol: (new esri.symbol.CartographicLineSymbol(esri.symbol.CartographicLineSymbol.STYLE_SOLID, new esri.Color("#FF0000"), 3.45, esri.symbol.CartographicLineSymbol.CAP_ROUND, esri.symbol.CartographicLineSymbol.JOIN_MITER, 5)),
            label: "50,000 - 99,999",
            field: "ADT_CUR"
          });
          theRenderer.addBreak({
            minValue: 100000,
            maxValue: 150000,
            symbol: (new esri.symbol.CartographicLineSymbol(esri.symbol.CartographicLineSymbol.STYLE_SOLID, new esri.Color("#FF0000"), 5.4, esri.symbol.CartographicLineSymbol.CAP_ROUND, esri.symbol.CartographicLineSymbol.JOIN_MITER, 5)),
          ...
          theRenderer.addBreak({
            minValue: 150000,
            maxValue: 200000,
            symbol: (new esri.symbol.CartographicLineSymbol(esri.symbol.CartographicLineSymbol.STYLE_SOLID, new esri.Color("#FF0000"), 7.35, esri.symbol.CartographicLineSymbol.CAP_ROUND, esri.symbol.CartographicLineSymbol.JOIN_MITER, 5)),
          ...
          theRenderer.addBreak({
            minValue: 200000,
            maxValue: 250000,
            symbol: (new esri.symbol.CartographicLineSymbol(esri.symbol.CartographicLineSymbol.STYLE_SOLID, new esri.Color("#FF0000"), 9.3, esri.symbol.CartographicLineSymbol.CAP_ROUND, esri.symbol.CartographicLineSymbol.JOIN_MITER, 5)),
          ...
          theRenderer.addBreak({
            minValue: 250000,
            maxValue: Infinity,
            symbol: (new esri.symbol.CartographicLineSymbol(esri.symbol.CartographicLineSymbol.STYLE_SOLID, new esri.Color("#FF0000"), 11.25, esri.symbol.CartographicLineSymbol.CAP_ROUND, esri.symbol.CartographicLineSymbol.JOIN_MITER, 5)),
            label: ">= 250,000",
            field: "ADT_CUR"
          });
          ...
          futureTrafficLyr = new FeatureLayer(projectsUrl, {
            id: "Future_Traffic",
            visible: true,
            outFields: ["RIA_RTE_ID", "ADT_CUR", "ADT_DESGN", "TRK_AADT_PCT", "ADT_YEAR", "DESGN_YR"],
            infoTemplate: infoTemplate
          });
        ...
        layerInfo.push({
          layer: futureTrafficLyr,
          title: "Future Traffic and Percent Trucks.  Future is calculated as 2% growth rate and percent truck is 24 hour."
        });
      }
      ...
      if (activeOverlaysArray.indexOf("Railroads") > -1) {
        ...
        if (railRoadLyr) {
          ...
          railRoadLyr = new FeatureLayer(projectsUrl, {
            id: "Railroads",
            visible: true,
            outFields: ["RR_COMPANY", "RR_ABRVN", "RR_STATUS", "RR_TYP", "RR_NBR"],
            infoTemplate: infoTemplate
          });
      ...
      if (activeOverlaysArray.indexOf("Non_Attainment") > -1) {
        ...
        if (nonAttainLyr) {
          ...
          nonAttainLyr = new FeatureLayer(projectsUrl, {
            id: "Non_Attainment",
            visible: true,
            outFields: ["FIPS", "REGION", "STATUS"],
            infoTemplate: infoTemplate
          });
        ...
        layerInfo.push({
          layer: nonAttainLyr,
          title: "Non-Attainment or Near Non-Attainment Areas"
        });
      }
      ...
      if (activeOverlaysArray.indexOf("Texas_Trunk") > -1) {
        ...
        if (txTrunkLyr) {
          ...
          theRenderer.addBreak({
            minValue: -Infinity,
            maxValue: 50000,
            symbol: (new esri.symbol.SimpleFillSymbol(esri.symbol.SimpleFillSymbol.STYLE_SOLID, new esri.symbol.SimpleLineSymbol(esri.symbol.SimpleLineSymbol.STYLE_DOT, new esri.Color([230, 152, 0]), 2.25), new esri.Color([182, 137, 48, 0.50]))),
            label: "Small Urban Area",
            field: "POP2010"
          });
          ...
          theRenderer.addBreak({
            minValue: 50000,
            maxValue: Infinity,
            symbol: (new esri.symbol.SimpleFillSymbol(esri.symbol.SimpleFillSymbol.STYLE_SOLID, new esri.symbol.SimpleLineSymbol(esri.symbol.SimpleLineSymbol.STYLE_DOT, new esri.Color([0, 77, 168]), 2.25), new esri.Color([90, 147, 168, 0.50]))),
            label: "Urbanized Area",
            field: "POP2010"
          });


          txTrunkLyr = new FeatureLayer(projectsUrl, {
            id: "Texas_Trunk",
            visible: true,
            outFields: ["RTE_RB_NM", "TRUNK_TYP"],
            infoTemplate: infoTemplate
          });
          UZALyr = new FeatureLayer("http://services.arcgis.com/KTcxiTD9dsQw4r7Z/arcgis/rest/services/TxDOT_Urbanized_Areas/FeatureServer/0", {
            outFields: ["*"],
            infoTemplate: UZAInfoTemplate
          });
        ...
        layerInfo.push({
          layer: UZALyr,
          title: "2010 Adjusted Urban Areas"
        }, {
          layer: txTrunkLyr,
      ...
      if (activeOverlaysArray.indexOf("Maintenance_Offices") > -1) {
        ...
        if (maintOfficeLyr) {
          ...
          maintOfficeLyr = new FeatureLayer(projectsUrl, {
            id: "Maintenance_Offices",
            visible: true,
            outFields: ["OFFICE_NM", "MNT_SEC_NBR", "DIST_NM"],
            infoTemplate: infoTemplate
          });
        ...
        layerInfo.push({
          layer: maintOfficeLyr,
          title: "TxDOT Maintenance Offices"
        });
      }
      ...
      if (activeOverlaysArray.indexOf("Functional_Classification") > -1) {
        ...
        if (FCLyr) {
          ...
          FCLyr = new FeatureLayer(projectsUrl, {
            id: "Functional_Classification",
            visible: true,
            outFields: ["FAC_NAM", "PROPOSED", "F_SYSTEM", "FC_DESC", "RU"],
            infoTemplate: infoTemplate
          });
      ...
      if (activeOverlaysArray.indexOf("Natl_Freight_Network") > -1) {
        ...
        if (NatlFreightLyr) {
          ...
          NatlFreightLyr = new FeatureLayer(projectsUrl, {
            id: "National Highway Freight Network",
            visible: true,
            outFields: ["RTE_RB_NM", "NATL_FRT"],
            infoTemplate: infoTemplate
          });
        ...
        layerInfo.push({
          layer: NatlFreightLyr,
          title: "National Highway Freight Network"
        });
      }
      ...
      if (activeOverlaysArray.indexOf("TX_Freight_Network") > -1) {
        ...
        if (TXFreightLyr) {
          ...
          TXFreightLyr = new FeatureLayer(projectsUrl, {
            id: "Freight Network",
            visible: true,
            outFields: ["RTE_RB_NM", "TIER"],
            infoTemplate: infoTemplate
          });
        ...
        layerInfo.push({
          layer: TXFreightLyr,
          title: "Texas Highway Freight Network"
        });
      }
      ...
      if (activeOverlaysArray.indexOf("Vertical_Clearance") > -1) {
        ...
        if (VerticalLyr) {
          ...
          VerticalLyr = new FeatureLayer(projectsUrl, {
      ...
      if (activeOverlaysArray.indexOf("Permanent_Stations") > -1) {
        ...
        if (permStationLyr) {
          ...
          permStationLyr = new FeatureLayer(projectsUrl, {
            id: "Permanent Stations",
            visible: true,
            outFields: ["T_FLAG", "STATION_TYPE"],
            infoTemplate: infoTemplate
          });
        ...
        layerInfo.push({
          layer: permStationLyr,
          title: "Permanent Count Stations"
        });
      }
      ...
      if (activeOverlaysArray.indexOf("AADT") > -1) {
        ...
        if (aadtLyr) {
          ...
          tableText += "<tr><td>AADT 2016</td><td>${F2016_TRAF:NumberFormat}</td></tr>";
          ...
          aadtLyr = new FeatureLayer(projectsUrl, {
            id: "AADT",
            visible: true,
            outFields: ["T_FLAG", "T_SITE_ID", "F2016_TRAF", "F2015_TRAF", "F2014_TRAF", "F2013_TRAF", "F2012_TRAF", "F2011_TRAF", "F2010_TRAF", "F2009_TRAF", "F2008_TRAF", "F2007_TRAF"],
            infoTemplate: infoTemplate
          });
          ...
          labelLayerAADT = new LabelLayer({
            id: "AADT_Labels"
          });
          labelLayerAADT.addFeatureLayer(aadtLyr, label_renderer, "{F2016_TRAF}");
        ...
        layerInfo.push({
          layer: aadtLyr,
          title: "Annual Average Daily Traffic - Count Location"
        });
      }
      ...
      if (activeOverlaysArray.indexOf("Reference_Markers") > -1) {
        ...
        if (MarkerLyr) {
          ...
          tableText += "<tr><td>DFO</td><td>${DFO:NumberFormat(places:3)}</td></tr>";
          tableText += "</table>";

          var infoTemplate = new InfoTemplate("Reference Markers:", tableText);
          var projectsUrl = "http://services.arcgis.com/KTcxiTD9dsQw4r7Z/arcgis/rest/services/TxDOT_Reference_Markers/FeatureServer/0";

          MarkerLyr = new FeatureLayer(projectsUrl, {
            id: "Reference Marker",
            visible: true,
            outFields: ["*"],
            infoTemplate: infoTemplate
          ...
          var label_renderer = new SimpleRenderer(label_symbol);
          labelLayerMarker = new LabelLayer({
            id: "Marker_Labels"
          });
      ...
      if (activeOverlaysArray.indexOf("State_House_Districts") > -1) {
        ...
        if (StateHouseLyr) {
          ...
          StateHouseLyr = new FeatureLayer(projectsUrl, {
            id: "State House District",
            visible: true,
            outFields: ["DIST_NBR", "REP_NM"],
            infoTemplate: infoTemplate
          });
      ...
      if (activeOverlaysArray.indexOf("State_Senate_Districts") > -1) {
        ...
        if (StateSenateLyr) {
          ...
          StateSenateLyr = new FeatureLayer(projectsUrl, {
        ...
        layerInfo.push({
          layer: StateSenateLyr,
          title: "Texas Senate Districts"
        });
      }
      ...
      if (activeOverlaysArray.indexOf("US_House_Districts") > -1) {
        ...
        if (USHouseLyr) {
          ...
          USHouseLyr = new FeatureLayer(projectsUrl, {
            id: "US House District",
            visible: true,
            outFields: ["DIST_NBR", "REP_NM"],
            infoTemplate: infoTemplate
          });
      ...
      if (activeOverlaysArray.indexOf("NHS") > -1) {
        ...
        if (NHSLyr) {
          ...
          NHSLyr = new FeatureLayer(projectsUrl, {
            id: "NHS",
            visible: true,
            outFields: ["RTE_NM", "Sys_Type", "NHS_CD", "NHS_TYPE"],
            infoTemplate: infoTemplate
          });
          map.addLayer(NHSLyr);
        }
      ...
      if (activeOverlaysArray.indexOf("Area_Offices") > -1) {
        ...
        if (areaOfficeLyr) {
          ...
          areaOfficeLyr = new FeatureLayer(projectsUrl, {
            id: "Area_Offices",
            visible: true,
            outFields: ["OFFICE_NM", "DIST_NM"],
            infoTemplate: infoTemplate
          });
      ...
      if (activeOverlaysArray.indexOf("COG") > -1) {
        ...
        if (COGLyr) {
          ...
          COGLyr = new FeatureLayer(projectsUrl, {
            id: "COG",
            visible: true,
            outFields: ["COG_NM", "COG_ABRVN"],
            infoTemplate: infoTemplate
          });
          map.addLayer(COGLyr);
        }
      ...
      if (activeOverlaysArray.indexOf("MPO") > -1) {
        ...
        if (MPOLyr) {
          ...
          MPOLyr = new FeatureLayer(projectsUrl, {
            id: "MPO",
            visible: true,
            outFields: ["MPO_NM", "MPO_LBL", "MPO_NBR"],
            infoTemplate: infoTemplate
          });
          map.addLayer(MPOLyr);
        }
      ...
      if (activeOverlaysArray.indexOf("Control_Section") > -1) {
        ...
        if (controlSectionLyr) {
          ...
          controlSectionLyr = new FeatureLayer(projectsUrl, {
            id: "Control_Section",
            visible: true,
            outFields: ["CTRL_SECT_LN_NBR", "RTE_RB_NM", "BEGIN_MPT", "END_MPT"],
            infoTemplate: infoTemplate
        ...
        layerInfo.push({
          layer: controlSectionLyr,
          title: "Control Section segment colors only serve to distinguish one segment from the next."
        });
      }
      ...
      if (activeOverlaysArray.indexOf("Energy_Sector") > -1) {
        ...
        if (energySectorLyr) {
          ...
          tableText = "";
          tableText += "<table border=1 cellspacing=0 cellpadding=3 style='font-size:small'>";
          tableText += "<tr><td>Route</td><td>${RTE_RB_NM}</td></tr>";
          tableText += "</table>";
          ...
          energySectorLyr = new FeatureLayer(projectsUrl, {
            id: "Energy_Sector",
            visible: true,
            outFields: ["RTE_RB_NM", "COUNTY", "ENRGY_SCTR"],
            infoTemplate: infoTemplate
          });
      ...
      if (activeOverlaysArray.indexOf("Shale_Plays") > -1) {
        ...
        if (shalePlaysLyr) {
          ...
          tableText = "";
          tableText += "<table border=1 cellspacing=0 cellpadding=3 style='font-size:small'>";
          tableText += "<tr><td>Shale Play Name</td><td>${Shale_Play}</td></tr>";
          tableText += "</table>";

          tableText2 = "";
          tableText2 += "<table border=1 cellspacing=0 cellpadding=3 style='font-size:small'>";
          tableText2 += "<tr><td>Basin Name</td><td>${NAME}</td></tr>";
          tableText2 += "</table>";

          var infoTemplate = new InfoTemplate("Shale Play:", tableText);
          var infoTemplate2 = new InfoTemplate("Basin:", tableText2);
          // var projectsUrl1 = "http://services2.arcgis.com/FiaPA4ga0iQKduv3/arcgis/rest/services/Plays/FeatureServer";
          // var projectsUrl2 = "http://services2.arcgis.com/FiaPA4ga0iQKduv3/arcgis/rest/services/Basins/FeatureServer";


          shalePlaysLyr = new FeatureLayer("http://services2.arcgis.com/FiaPA4ga0iQKduv3/arcgis/rest/services/Plays/FeatureServer/0", {
            id: "Shale_Plays",
            visible: true,
            outFields: ["*"],
            infoTemplate: infoTemplate
          });
          shaleBasinsLyr = new FeatureLayer("http://services2.arcgis.com/FiaPA4ga0iQKduv3/arcgis/rest/services/Basins/FeatureServer/0", {
            id: "Shale_Basins",
            visible: true,
            outFields: ["*"],
            infoTemplate: infoTemplate2
          });
          map.addLayers([shaleBasinsLyr, shalePlaysLyr]);

          shalePlaysLyr.on("load", function() {
            shalePlaysLyr.renderer.setOpacityInfo({
              opacityValues: [0.5]
            });
          });
          shaleBasinsLyr.on("load", function() {
            shaleBasinsLyr.renderer.setOpacityInfo({
              opacityValues: [0.5]
            });
          });
      ...
      if (activeOverlaysArray.indexOf("Top_100") > -1) {
        ...
        if (top100Lyr) {
          ...
          top100Lyr = new FeatureLayer(projectsUrl, {
            id: "Top 100:",
            visible: true,
            outFields: ["RANK", "TRK_RANK", "SEG_ID", "RD_NM", "DLAY_MILE", "TRK_DLAY", "TCI", "COST_DLAY", "COST_TRK", "DIST_NM", "YR"],
            infoTemplate: infoTemplate
          });
          ...
          var label_renderer = new SimpleRenderer(label_symbol);
          labelLayerTop100 = new LabelLayer({
            id: "Top_100_Labels"
          });
          labelLayerTop100.addFeatureLayer(top100Lyr, label_renderer, "{RANK}", {
            lineLabelPosition: "InLine",
            labelRotation: false
          });
      ...
      if (activeOverlaysArray.indexOf("Roadway_Inventory") > -1) {
        ...
        if (rdwyInvLyr) {
          ...
          function getTextContent(graphic) {
            ...
            tableText += "<tr><td>VMT</td><td>" + rndVMT + "</td></tr>";
            tableText += "</table>";
            return tableText;
          }
          ...
          rdwyInvLyr = new FeatureLayer(projectsUrl, {
            id: "Roadway_Inventory",
            visible: true,
            outFields: ["*"],
            infoTemplate: infoTemplate
          });
      ...
      if (activeOverlaysArray.indexOf("Cemetery") > -1) {
        ...
        if (cemeteryLyr) {
          ...
          cemeteryLyr = new FeatureLayer(projectsUrl, {
            id: "Cemetery",
            visible: true,
            outFields: ["CEMETERY_NM", "CNTY_NM", "Zoom"],
            infoTemplate: infoTemplate
          });
      ...
      if (activeOverlaysArray.indexOf("RMA") > -1) {
        ...
        if (RMAlyr) {
          ...
          tableText = "";
          tableText += "<table border=1 cellspacing=0 cellpadding=3 style='font-size:small'>";
          tableText += "<tr><td>Name</td><td>${Label}</td></tr>";
          tableText += "</table>";

          var infoTemplate = new InfoTemplate("Regional Mobility Authority:", tableText);
          var projectsUrl = "http://services.arcgis.com/KTcxiTD9dsQw4r7Z/arcgis/rest/services/Texas_Regional_Mobility_Authorities/FeatureServer/0";

          RMAlyr = new FeatureLayer(projectsUrl, {
            id: "RMA",
            visible: true,
            outFields: ["Label"],
            infoTemplate: infoTemplate
        ...
        layerInfo.push({
          layer: RMAlyr,
          title: "Regional Mobility Authority (RMA)"
        });
      }
      if (activeOverlaysArray.indexOf("Tolls") > -1) {
        ...
        if (tollLyr) {
          ...
          tableText = "";
          tableText += "<table border=1 cellspacing=0 cellpadding=3 style='font-size:small'>";
          tableText += "<tr><td>Toll Name</td><td>${TOLL_NM}</td></tr>";
          tableText += "<tr><td>Segment or Alternate Name</td><td>${ALT_NM_SEG}</td></tr>";
          tableText += "<tr><td>Route Name</td><td>${RTE_RB_NM}</td></tr>";
          tableText += "<tr><td>Status</td><td>${TOLL_STAT}</td></tr>";
          tableText += "<tr><td>Comment</td><td>${CMNT}</td></tr>";
          tableText += "</table>";
          ...
          tollLyr = new FeatureLayer(projectsUrl, {
            id: "Tolls",
            visible: true,
            outFields: ["TOLL_NM", "RTE_RB_NM", "TOLL_STAT", "ALT_NM_SEG", "CMNT"],
            infoTemplate: infoTemplate
          });
  ...
  function addInventoryData() {
    ...
    projectsRI = new FeatureLayer(projectsUrl, {
      id: "RoadwayInventory",
      visible: false,
      outFields: ["HWY", "ADT_CUR", "ADT_DESGN", "NUM_LANES", "SPD_MAX", "TRK_AADT_PCT", "ROW_W_USL", "SUR_W", "RB_WID", "MED_WID", "HSYS", "ADT_YEAR"],
      infoTemplate: infoTemplate
    });
  ...
  // Events and functions for enabling the survey option
